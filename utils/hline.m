function [hhh, ttt]=hline(y,in1,in2)
...
if length(y)>1  % vector input
    for I=1:length(y)
        ...
        [h(I), t(I)]=hline(y(I),linetype,label);
    end
else
    switch nargin
    case 1
    ...
    if ~isempty(label)
        ...
        yunit=(y-yy(1))/yrange;
        if yunit<0.2
            t = text(x(1)+0.02*(x(2)-x(1)),y+0.02*yrange,label,'color',get(h,'color'));
        else
            t = text(x(1)+0.02*(x(2)-x(1)),y-0.02*yrange,label,'color',get(h,'color'));
        end
    else
